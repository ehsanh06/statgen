...
function registerPartials() {
    const files = fs.readdirSync(config().paths.includes);
    files.forEach((file) => {
...
function createIndex() {
    createPage({
        layout: 'index',
        path: (config().paths.site == '') ? '' : config().paths.site + '/',
...
function createArchive() {
    if(!config().archive.show) return;

    let path = (config().paths.site == '') ? '' : config().paths.site + '/';
    createPage({
        layout: 'archive',
        path: path + config().archive.name + '/',
