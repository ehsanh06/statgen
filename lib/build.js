...
handlebars.registerPartial('nav', fs.readFileSync(config.path.includes + 'nav.hbs', 'utf8'));
handlebars.registerPartial('header', fs.readFileSync(config.path.includes + 'header.hbs', 'utf8'));
handlebars.registerPartial('footer', fs.readFileSync(config.path.includes + 'footer.hbs', 'utf8'));
handlebars.registerPartial('recent', fs.readFileSync(config.path.includes + 'recent.hbs', 'utf8'));

function createPage(data) {
    var layout = fs.readFileSync(config.path.layouts + data.layout + '.html', 'utf-8');
...
function createAllPages() {
    ...
    _.each(siteData.posts, function (post) {
        ...
        createPage({
            layout: post.layout,
            path: config.path.dist + post.year + '/' + post.slug + '/',
            pageData: pageData
        });
    });
